...
V3 = add sigmoid, clean up starting prob and reading from test.
"""
...
def viterbi(seq, trans_matrix, emit_matrix, start_matrix):
    ...
    for _ in range(1, len(seq)):
        ...
        current_prob = {}
        for state in states:
            V, previous_state = max(((sigmoid(prev_prob[prev_state] * trans_matrix.loc[prev_state, state] * emit_matrix.at[state, seq[_]]), prev_state) for prev_state in states))
...
def viterbi2(seq, states, start_matrix, trans_matrix, emit_matrix):  # Wiki implementation
