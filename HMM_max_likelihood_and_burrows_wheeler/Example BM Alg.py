...
def backward_probs():
    ...
    print("start_state: " + str(start_state))
    start_state = np.multiply(start_state, start_probs)
    print("start_state2: " + str(start_state))
    print("seq_syms_translated: " + str(sequence_syms[test_sequence[0]]))
...
for iteration in range(2000):
    ...
    for j in range(len(states)): #states
        for i in range(len(sequence)): #seq
            ...
            print("num_b, denom_b")
            print(numerator_b, denomenator_b)
            if (denomenator_b == 0):
                b[j,i] = 0
            else:
